  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold">テンプレ設定</h1>
                ...
                <div className="space-y-2">
                  {questions.map((q: PreQuestion, i: number) => (
                    <div key={i} className={`flex items-center gap-3 p-2 rounded ${q.isActive ? "" : "opacity-50"}`}>
                      <span className="text-xs text-muted-foreground w-6">{q.order}.</span>
                      <span className="flex-1 text-sm">{q.text}</span>
                      <span className={`text-xs px-2 py-0.5 rounded ${q.isActive ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}>
                        {q.isActive ? "有効" : "無効"}
                      </span>
                    </div>
                  ))}
                </div>
              ) : <p className="text-sm text-muted-foreground">質問がありません</p>;
            })()}
          </section>
                ...
                {(versions.data as any[]).map((v: any) => (
                  <div key={v.id} className="flex items-center justify-between p-2 border border-border rounded text-sm">
                    <div>
                      ...
                      {v.restoredAt && <span className="ml-2 text-xs text-blue-600">復元済</span>}
                    </div>
                    <button
              ...
              {form.preQuestions.map((q, i) => (
                <div key={i} className="flex items-start gap-2 p-3 border border-border rounded">
                  <div className="flex flex-col gap-1">
                    <button onClick={() => moveQuestion(i, "up")} disabled={i === 0} className="text-xs disabled:opacity-30">▲</button>
                    <button onClick={() => moveQuestion(i, "down")} disabled={i === form.preQuestions.length - 1} className="text-xs disabled:opacity-30">▼</button>
                  </div>
                  <input
                    type="text"
                    value={q.text}
                  ...
                  <button onClick={() => removeQuestion(i)} className="text-destructive text-sm hover:underline">削除</button>
                </div>
              ))}
            </div>
          </section>
